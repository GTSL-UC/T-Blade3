subroutine thk_ctrl_gen_spl(uthk, thk, n, nknt, d_0, d_1, dAdE, dBdE, dCdE, dDdE, dEdE, &
    ...
    integer :: i, j, k, nun, nseg, thkm_i, row, col, fail_flag, row_Emin, row_D1min, seg_end, &
        d2v_sign, d3v_sign, nseg1, nseg2, nmax, nmin, line_flag
                    ...
                    do p = 1, 4
                        if (aimag(root(p)) .eq. 0.0D0) then
                            if (dble(root(p)) .le. 1.0D0 .and. dble(root(p)) .ge. 0.0D0) then
                                t_spl(k) = real(root(p),8)!dble(root(p))
                                exit
                            endif
                        endif
                        if (p .eq. 4) then
