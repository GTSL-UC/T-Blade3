...
angle = -1*(y_cp(1)*(x_cp(1)*(t**6/72 - t**5/12 + 5*t**4/24 - 5*t**3/18 + 5*t**2/24 - t/12) + x_cp(2)*(-t**6/24 + 13*t**5/60 - 7*t**4/16 + 5*t**3/12 - t**2/6) + x_cp(3)*(t**6/24 - 11*t**5/60 + 7*t**4/24 - t**3/6 - t**2/24 + t/12) + x_cp(4)*(-t**6/72 + t**5/20 - t**4/16 + t**3/36)) + &
y_cp(2)*(x_cp(1)*(-t**6/24 + t**5/5 - 5*t**4/16 + t**3/18 + t**2/3 - t/3) + x_cp(2)*(t**6/8 - t**5/2 + t**4/2 + t**3/3 - 2*t**2/3) + x_cp(3)*(-t**6/8 + 2*t**5/5 - 3*t**4/16 - t**3/2 + t**2/3 + t/3) + x_cp(4)*(t**6/24 - t**5/10 + t**3/9)) + &
y_cp(3)*(x_cp(1)*(t**6/24 - 3*t**5/20 + t**4/8 + t**3/18 - t**2/24 - t/12) + x_cp(2)*(-t**6/8 + 7*t**5/20 - t**4/16 - t**3/4 - t**2/6) + x_cp(3)*(t**6/8 - t**5/4 - t**4/8 + t**3/6 + 5*t**2/24 + t/12) + x_cp(4)*(-t**6/24 + t**5/20 + t**4/16 + t**3/36)) + &
y_cp(4)*(t**6*x_cp(4)/72 + x_cp(1)*(-t**6/72 + t**5/30 - t**4/48) + x_cp(2)*(t**6/24 - t**5/15) + x_cp(3)*(-t**6/24 + t**5/30 + t**4/48)) ) &
+ angle0   
...
camber = -1*(y_cp(1)*(x_cp(1)*(x_cp(1)*(-t**9/1296 + t**8/144 - t**7/36 + 7*t**6/108 - 7*t**5/72 + 7*t**4/72 - t**3/16 + t**2/48) + x_cp(2)*(t**9/432 - 11*t**8/576 + 23*t**7/336 - 5*t**6/36 + 25*t**5/144 - 13*t**4/96 + t**3/18) + x_cp(3)*(-t**9/432 + 5*t**8/288 - t**7/18 + 7*t**6/72 - 7*t**5/72 + 7*t**4/144 + t**3/144 - t**2/48) + x_cp(4)*(t**9/1296 - t**8/192 + 5*t**7/336 - 5*t**6/216 + t**5/48 - t**4/96)) + x_cp(2)*(x_cp(1)*(t**9/432 - 3*t**8/160 + 73*t**7/1120 - 181*t**6/1440 + 23*t**5/160 - 3*t**4/32 + t**3/36) + x_cp(2)*(-t**9/144 + 49*t**8/960 - 523*t**7/3360 + t**6/4 - 13*t**5/60 + t**4/12) + x_cp(3)*(t**9/144 - 11*t**8/240 + 409*t**7/3360 - 229*t**6/1440 + 43*t**5/480 + t**4/96 - t**3/36) + x_cp(4)*(-t**9/432 + 13*t**8/960 - t**7/32 + 5*t**6/144 - t**5/60)) + x_cp(3)*(x_cp(1)*(-t**9/432 + t**8/60 - t**7/20 + 7*t**6/90 - 7*t**5/120 + 5*t**3/144 - t**2/48) + x_cp(2)*(t**9/144 - 43*t**8/960 + 193*t**7/1680 - 5*t**6/36 + 13*t**5/240 + 5*t**4/96 - t**3/18) + x_cp(3)*(-t**9/144 + 19*t**8/480 - 3*t**7/35 + 3*t**6/40 + t**5/120 - t**4/16 + t**3/48 + t**2/48) + x_cp(4)*(t**9/432 - 11*t**8/960 + t**7/48 - t**6/72 - t**5/240 + t**4/96)) + x_cp(4)*(x_cp(1)*(t**9/1296 - 7*t**8/1440 + 127*t**7/10080 - 73*t**6/4320 + 17*t**5/1440 - t**4/288) + x_cp(2)*(-t**9/432 + 37*t**8/2880 - 31*t**7/1120 + t**6/36 - t**5/90) + x_cp(3)*(t**9/432 - t**8/90 + 197*t**7/10080 - 19*t**6/1440 - t**5/1440 + t**4/288) + x_cp(4)*(-t**9/1296 + t**8/320 - t**7/224 + t**6/432))) + &
y_cp(2)*(x_cp(1)*(x_cp(1)*(t**9/432 - 17*t**8/960 + 181*t**7/3360 - 317*t**6/4320 + 13*t**5/1440 + 17*t**4/144 - t**3/6 + t**2/12) + x_cp(2)*(-t**9/144 + 23*t**8/480 - 139*t**7/1120 + 17*t**6/144 + 7*t**5/90 - 7*t**4/24 + 2*t**3/9) + x_cp(3)*(t**9/144 - 41*t**8/960 + 311*t**7/3360 - 71*t**6/1440 - 173*t**5/1440 + 31*t**4/144 - t**3/18 - t**2/12) + x_cp(4)*(-t**9/432 + t**8/80 - 5*t**7/224 + t**6/216 + t**5/30 - t**4/24)) + x_cp(2)*(x_cp(1)*(-t**9/144 + 3*t**8/64 - 13*t**7/112 + 7*t**6/72 + t**5/12 - 5*t**4/24 + t**3/9) + x_cp(2)*(t**9/48 - t**8/8 + t**7/4 - t**6/12 - t**5/3 + t**4/3) + x_cp(3)*(-t**9/48 + 7*t**8/64 - 19*t**7/112 - t**6/24 + 19*t**5/60 - t**4/8 - t**3/9) + x_cp(4)*(t**9/144 - t**8/32 + t**7/28 + t**6/36 - t**5/15)) + x_cp(3)*(x_cp(1)*(t**9/144 - 13*t**8/320 + 89*t**7/1120 - 11*t**6/480 - 11*t**5/96 + 5*t**4/48 + t**3/18 - t**2/12) + x_cp(2)*(-t**9/48 + 17*t**8/160 - 173*t**7/1120 - t**6/16 + 3*t**5/10 - t**4/24 - 2*t**3/9) + x_cp(3)*(t**9/48 - 29*t**8/320 + 99*t**7/1120 + 61*t**6/480 - 7*t**5/32 - 5*t**4/48 + t**3/6 + t**2/12) + x_cp(4)*(-t**9/144 + t**8/40 - 3*t**7/224 - t**6/24 + t**5/30 + t**4/24)) + x_cp(4)*(x_cp(1)*(-t**9/432 + 11*t**8/960 - 29*t**7/1680 - t**6/1080 + t**5/45 - t**4/72) + x_cp(2)*(t**9/144 - 7*t**8/240 + t**7/35 + t**6/36 - 2*t**5/45) + x_cp(3)*(-t**9/144 + 23*t**8/960 - 19*t**7/1680 - 13*t**6/360 + t**5/45 + t**4/72) + x_cp(4)*(t**9/432 - t**8/160 + t**6/108))) + &
y_cp(3)*(x_cp(1)*(x_cp(1)*(-t**9/432 + 7*t**8/480 - t**7/30 + 31*t**6/1080 + t**5/360 - t**4/144 - t**3/48 + t**2/48) + x_cp(2)*(t**9/144 - 37*t**8/960 + 39*t**7/560 - t**6/36 - 5*t**5/144 - t**4/96 + t**3/18) + x_cp(3)*(-t**9/144 + t**8/30 - 19*t**7/420 - t**6/180 + 13*t**5/360 + t**4/36 - 5*t**3/144 - t**2/48) + x_cp(4)*(t**9/432 - 3*t**8/320 + t**7/112 + t**6/216 - t**5/240 - t**4/96)) + x_cp(2)*(x_cp(1)*(t**9/144 - 3*t**8/80 + 71*t**7/1120 - 3*t**6/160 - 13*t**5/480 - t**4/96 + t**3/36) + x_cp(2)*(-t**9/48 + 31*t**8/320 - 127*t**7/1120 - t**6/24 + t**5/20 + t**4/12) + x_cp(3)*(t**9/48 - 13*t**8/160 + 61*t**7/1120 + 13*t**6/160 - t**5/160 - 7*t**4/96 - t**3/36) + x_cp(4)*(-t**9/144 + 7*t**8/320 - t**7/224 - t**6/48 - t**5/60)) + x_cp(3)*(x_cp(1)*(-t**9/144 + t**8/32 - t**7/28 - t**6/72 + t**5/40 + t**4/48 - t**3/144 - t**2/48) + x_cp(2)*(t**9/48 - 5*t**8/64 + 5*t**7/112 + t**6/12 - t**5/240 - 7*t**4/96 - t**3/18) + x_cp(3)*(-t**9/48 + t**8/16 - t**6/12 - t**5/24 + t**4/24 + t**3/16 + t**2/48) + x_cp(4)*(t**9/144 - t**8/64 - t**7/112 + t**6/72 + t**5/48 + t**4/96)) + x_cp(4)*(x_cp(1)*(t**9/432 - t**8/120 + 19*t**7/3360 + 17*t**6/4320 - t**5/1440 - t**4/288) + x_cp(2)*(-t**9/144 + 19*t**8/960 - t**7/1120 - t**6/72 - t**5/90) + x_cp(3)*(t**9/144 - 7*t**8/480 - 31*t**7/3360 + 11*t**6/1440 + 17*t**5/1440 + t**4/288) + x_cp(4)*(-t**9/432 + t**8/320 + t**7/224 + t**6/432))) + &
y_cp(4)*(x_cp(1)*(x_cp(1)*(t**9/1296 - 11*t**8/2880 + 73*t**7/10080 - t**6/160 + t**5/480) + x_cp(2)*(-t**9/432 + 7*t**8/720 - 47*t**7/3360 + t**6/144) + x_cp(3)*(t**9/432 - 23*t**8/2880 + 83*t**7/10080 - t**6/1440 - t**5/480) + x_cp(4)*(-t**9/1296 + t**8/480 - t**7/672)) + x_cp(2)*(x_cp(1)*(-t**9/432 + 3*t**8/320 - t**7/80 + t**6/180) + x_cp(2)*(t**9/144 - 11*t**8/480 + 2*t**7/105) + x_cp(3)*(-t**9/144 + 17*t**8/960 - 11*t**7/1680 - t**6/180) + x_cp(4)*(t**9/432 - t**8/240)) + x_cp(3)*(x_cp(1)*(t**9/432 - 7*t**8/960 + t**7/160 + t**6/1440 - t**5/480) + x_cp(2)*(-t**9/144 + t**8/60 - 17*t**7/3360 - t**6/144) + x_cp(3)*(t**9/144 - 11*t**8/960 - 3*t**7/1120 + t**6/160 + t**5/480) + x_cp(4)*(-t**9/432 + t**8/480 + t**7/672)) + x_cp(4)*(t**9*x_cp(4)/1296 + x_cp(1)*(-t**9/1296 + t**8/576 - t**7/1008) + x_cp(2)*(t**9/432 - t**8/288) + x_cp(3)*(-t**9/432 + t**8/576 + t**7/1008))) ) &
+ &
angle0*(x_cp(1)*(-t**3/6 + t**2/2 - t/2) + &
x_cp(2)*(t**3/2 - t**2) + &
x_cp(3)*(-t**3/2 + t**2/2 + t/2) + &
x_cp(4)*(t**3/6) ) &
+ &
camber0
