...
\begin{center}
\textbf{\large T-Blade3 v1.3.1 ReadMe}\\
by\\
Mayank Sharma\\
...
\noindent
Derivatives of $(x, y, z)$ blade sections with respect to the control points of several input parameters were added in version 1.3. New in version 1.3.1:
\begin{itemize}[leftmargin=*]
	\item Derivatives of the stagger\cite{KarthikThesis} as well as the derivatives of $(m', \theta)$ minimum throat\cite{AhmedPaper} for each spanwise section
\end{itemize}
...
\noindent 
There are two ways of obtaining T-Blade3 version 1.3.1:
...
\noindent
With T-Blade3 version 1.3.1, static builds for Linux, Unix and MacOS are no longer supported. The recommended method is to clone or download the T-Blade3 source code and compile it locally. Below are instructions to compile the code. Here, \$TBLADEROOT refers to the T-Blade3 top-level directory.
...
\noindent
The auxiliary input file \textit{\textcolor{blue}{spancontrolinputs.bladerow.dat}} (old format) is an older file which is still supported in version 1.3.1. Also see \textbf{\ref{auxinput1}}. It is used when the following switches are turned on in the main input file \textit{\textcolor{blue}{3dbgbinput.bladerow.dat}}:
\begin{itemize}
    \item Airfoil defined by curvature control (set to "\textbf{spanwise\_spline}")
\end{itemize}

\noindent 
The auxiliary input file \textit{\textcolor{blue}{controlinputs.bladerow.dat}} is an older file which is still supported in version 1.3.1. Also see \textbf{\ref{auxinput2}}. It is used when the following switches are turned on in the main input file \textit{\textcolor{blue}{3dbgbinput.bladerow.dat}}:
\begin{itemize}
...
\noindent
Output files created containing the derivatives of $(x,y,z)$ blade sections, stagger and $(m', \theta)$ minumum
throat with respect to control parameters of sweep, lean, inlet angle, exit angle, chord multiplier, mean-line
second derivative and modified NACA thickness.\\
\noindent
...
\textit{\textcolor{blue}{stagger\_ders\_parameter\_field.dat}} --- contains the derivatives of the stagger
of all spanwise sections with respect to all spanwise control points of a specific parameter\\
\textit{\textcolor{blue}{throat\_ders\_parameter\_field.dat}} --- contains the derivatives of the $(m', \theta)$
minimum throat of all spanwise sections with respect to all spanwise control points of a specific parameter\\
...
\noindent
This is an auxiliary input file for T-Blade3. This is an older file, which is still supported in T-Blade3 version 1.3.1 (refer \textbf{\ref{cmdargs}} for information on how to run T-Blade3 with this file) but it is recommended that the modified NACA four-digit thickness distribution is used with the current format of \textit{\textcolor{blue}{spancontrolinputs.bladerow.dat}}. This file is used when the thickness distribution switch (line 16) is set to "$\mathbf{1}$" and "$\mathbf{2}$". Following are the details of the file:
