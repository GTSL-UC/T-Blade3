...
!40
call thk_ctrl_gen_driver_aux (cname, dev_flag, sec, uthk, thk, n, nknt_spl, d_0_opt, d_1_opt, dAdE, dBdE, dCdE, dDdE, dEdE, &
    np, iknt_spl, niknt_spl, uthk_le, thk_le, n_le, nknt_le, dAdE_le, dBdE_le, dCdE_le, dDdE_le, dEdE_le, &
...
real, intent(in) :: uthk(n), thk(n), u_spl(np)
real, intent(out) :: out_coord(np, 12)
integer :: i, j, k, iter_max, niknt_spl, nknt_spl, nseg_spl, thkmx_i, &
           nmax, nmin, d3v_ang_flag, dev_flag, sl_flag
real :: d1v_te, d2v_te, d3v_te, d1v_le, d2v_le, d3v_le, u_thkmx, dseg, &
    F_opt, tol, Gn, d_0(3), d_1(3), d_0_init(3), d_0_opt(3), a, a0, a2, a3, &
    h1, h2, h3, F0, F1, F2, F3, d_1_opt(3), G(6), G_opt(6), G_temp(6)
...
integer :: i, d3v_ang_flag_le, d1_flag, np_fine, fail_flag
...
integer :: i, d3v_ang_flag, fail_flag
real :: F_spl, G_spl(6), G_spl_temp(6), F_spl_eps, eps, u_spl(np), out_coord_temp(np, 12), &
